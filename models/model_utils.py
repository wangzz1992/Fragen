...
class EdgeExpansion(nn.Module):
    ...
    """在__init__方法中，首先调用super().__init__()来初始化父类。然后，创建一个nn.Linear实例，命名为self.nn。
    nn.Linear是一个全连接层，接受一个输入特征数为1，输出特征数为edge_channels的线性变换，这里没有偏置项（bias=False）。"""
    ...
    def forward(self, edge_vector):
        ...
        return expansion
    """在forward方法中，首先对输入的边向量edge_vector进行归一化，这是通过除以其2范数实现的，这里加上一个小的常数1e-7是为了防止除以0。
    然后，将归一化后的边向量通过self.nn进行线性变换，并增加一个维度（unsqueeze(-1)），然后进行转置（transpose(1, -1)），得到expansion。
    最后，返回expansion。"""
...
class GaussianSmearing(nn.Module):
    def __init__(self, start=0.0, stop=10.0, num_gaussians=50):
        super().__init__()
        self.stop = stop
        offset = torch.linspace(start, stop, num_gaussians) # 用于生成一个等差数列
        self.coeff = -0.5 / (offset[1] - offset[0]).item()**2
        self.register_buffer('offset', offset)
    """在__init__方法中，首先调用了父类的初始化方法super().__init__()，然后定义了一些成员变量。self.stop用于存储高斯模糊的终止点。
    offset是一个等差数列，其起始点为start，终止点为stop，元素个数为num_gaussians。self.coeff是高斯模糊的系数，其值为-0.5 / (offset[1] - offset[0]).item()**2。
    最后，使用self.register_buffer('offset', offset)将offset注册为一个缓冲区，这样在进行设备迁移或序列化时，offset也会被一同处理。"""

    def forward(self, dist):
        dist = dist.clamp_max(self.stop)
        dist = dist.view(-1, 1) - self.offset.view(1, -1) # type: ignore
        return torch.exp(self.coeff * torch.pow(dist, 2))
    """在forward方法中，首先使用clamp_max方法将输入的距离dist限制在self.stop以内。
    然后，将dist和self.offset进行广播操作，得到一个新的距离矩阵。最后，使用torch.exp(self.coeff * torch.pow(dist, 2))计算了每个距离的高斯模糊值，并返回结果。"""
...
class GaussianSmearingVN(nn.Module):
    def __init__(self, start=0.0, stop=10.0, num_gaussians=64):
        ...
        self.coeff = -0.5 / delta.item()**2 # type: ignore
        self.register_buffer('offset', offset)
